...
def get_antinode_positions(position_a: tuple, position_b: tuple):
    ...
    if c1==c2:
        v_d = abs(r1 - r2)
        antinodes.append((min(r1, r2) - v_d, c1))
        antinodes.append((max(r1, r2) + v_d, c1))
    # Slope = 0
    elif r1==r2:
        h_d = abs(c1 - c2)
        antinodes.append((r1, min(c1, c2) - h_d))
        antinodes.append((r1, max(c1, c2) + h_d))
    else:
        slope = (r2 - r1) / (c2 - c1)
        v_d, h_d = abs(r1 - r2), abs(c1 - c2)

        if slope < 0:
            antinodes.append((min(r1, r2) - v_d, max(c1, c2) + h_d))
            antinodes.append((max(r1, r2) + v_d, min(c1, c2) - h_d))
        else:
            antinodes.append((min(r1, r2) - v_d, min(c1, c2) - h_d))
            antinodes.append((max(r1, r2) + v_d, max(c1, c2) + h_d))
