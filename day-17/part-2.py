...
def solution(lines: list[str]):
    ...
    for i in range(0, 1024):
        ...
        if len(output) not in len_set:
            period = i
            break
    
    bit_set_idx = len(program) - 1
    current_register_a = (period ** bit_set_idx)

    while bit_set_idx >= 0:
        machine = ThreeBitMachine(
            register_a = current_register_a,
            register_b=register_b,
            register_c=register_c,
            program=program
        )
        ...
        if output[bit_set_idx] == program[bit_set_idx]:
            bit_set_idx -= 1
        else:
            current_register_a += (period ** bit_set_idx)
    

    print(current_register_a)
